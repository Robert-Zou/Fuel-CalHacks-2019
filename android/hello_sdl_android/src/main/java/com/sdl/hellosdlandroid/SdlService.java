...
public class SdlService extends Service {
    ...
    private static final int TCP_PORT = 19838;
    ...
    private ComponentVolumeStatus currGasState = null;
    private List<FuelRange> currGasRange = null;
        ...
        if (sdlManager == null) {
            ...
            SdlManagerListener listener = new SdlManagerListener() {
                ...
                public void onStart() {
                    ...
                    // Define listener for car data
                    sdlManager.addOnRPCNotificationListener(FunctionID.ON_VEHICLE_DATA, new OnRPCNotificationListener() {
                        @Override
                        public void onNotified(RPCNotification notification) {

                            // Retrieve fuel level (if updated fuel level ever reaches below a threshold, handle it by calling fillGas)
                            ...
                            // Retrieve Fuel Level State (UNKNOWN/NORMAL/LOW/FAULT/ALERT/NOT_SUPPORTED)
                            if (onVehicleDataNotification.getFuelLevelState() != null) {
                                currGasState = onVehicleDataNotification.getFuelLevelState();
                                Log.i("SdlService", "Fuel state was updated to: " + currGasState);
                            }

                            // Retrieve Fuel Range
                            if (onVehicleDataNotification.getFuelRange() != null) {
                                currGasRange = onVehicleDataNotification.getFuelRange();
                                Log.i("SdlService", "Fuel state was updated to: " + currGasRange);
                            }

                            // Retrieve location (latitude degrees/longitude degrees)
                            if (onVehicleDataNotification.getGps() != null) {
                                currLatitude = onVehicleDataNotification.getGps().getLatitudeDegrees();
                                currLongitude = onVehicleDataNotification.getGps().getLongitudeDegrees();
                                Log.i("SdlService", "Latitude degrees was updated to: " + currLatitude);
                                Log.i("SdlService", "Longitude degrees was updated to: " + currLongitude);
                            }
                        }
                    });
    ...
    private void subscribeData() {
        ...
        subscribeRequest.setFuelLevelState(true);
        subscribeRequest.setFuelRange(true);
